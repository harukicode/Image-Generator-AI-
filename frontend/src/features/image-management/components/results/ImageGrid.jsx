...
export const ImageGrid = ({ images, onImageDelete }) => {
	...
	useEffect(() => {
		const filteredImages = images.filter((img) => {
			const filename = typeof img === "string" ? img.split("/").pop() : img.filename
	...
	const handleDelete = (image) => {
		const filename = typeof image === "string" ? image.split("/").pop() : image.filename || image.id?.toString()
		
		deleteImage(image, () => {
			setLocalImages((prev) =>
				prev.filter((img) => {
					const currentFilename = typeof img === "string" ? img.split("/").pop() : img.filename || img.id?.toString()
	...
	return (
		<div className="space-y-4">
			<div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 sm:gap-0 min-h-[40px]">
			...
			<div className="relative overflow-hidden min-h-[450px]">
				<AnimatePresence mode="wait" custom={direction}>
					...
					>
						<motion.div
							layout
							className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-2 p-1" // Adjusted gap and added padding
						>
