	...
	<div className="space-y-2">
		{!currentPrompt ? (
			...
			<div className="flex gap-2 flex-wrap">
				...
				<GenerateButton onClick={onGenerate} disabled={isGeneratingImages} isLoading={isGeneratingImages} />
			</div>
		)}
	</div>
)
...
const GenerateButton = ({ onClick, disabled, isLoading }) => (
	<motion.div className="flex-1" whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}>
		<Button onClick={onClick} disabled={disabled} className="w-full bg-[#6366F1] hover:bg-[#4F46E5] text-white">
			{isLoading && <LoadingSpinner />}
