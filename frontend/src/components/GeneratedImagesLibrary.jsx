...
function GeneratedImagesLibrary() {
  ...
  const handleDownload = async (filename) => {
    try {
      ...
      link.href = `http://localhost:3000/generated/${filename}`;
      
      // Получаем имя компании и номер файла из полного имени
      const name = filename.split('-')[0];
      const number = filename.match(/\d{2}\.png$/)[0];  // Берем последние две цифры перед .png
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
        {images.map((image) => (
          ...
          >
            ...
            />
            <div
              className="absolute inset-0 bg-black bg-opacity-60 opacity-0 group-hover:opacity-100 transition-opacity duration-200 flex items-center justify-center space-x-2">
              <Dialog>
